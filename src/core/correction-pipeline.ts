import { getCorrection, CorrectionResponse } from '../ai/openai-client';
...
const logError = async (userId: number, errorType: string): Promise<{ triggered: boolean }> => {
    const newCount = await incrementErrorCount(userId, errorType);
    console.log(`Logged error '${errorType}' for user ${userId}. New count: ${newCount}`);

    if (newCount >= REINFORCEMENT_THRESHOLD) {
        console.log(`Reinforcement triggered for user ${userId} on error type '${errorType}'`);
...
const generateReinforcementMessage = (errorType: string): string => {
    ...
    switch (errorType) {
        case 'tense':
            extraInfo = "Tenses tell you when an action happens (e.g., 'I walk' vs. 'I walked'). Let's focus on getting them right!";
            break;
        case 'preposition':
            extraInfo = "Prepositions like 'in', 'on', 'at' show relationships. They often require practice.";
            break;
        case 'word_choice':
            extraInfo = "Choosing the most accurate word is key. Similar words can have very different meanings.";
            break;
        default:
            extraInfo = "Grammar rules can be complex, but mastering them makes a big difference.";
    }

    return `🔔 You've made this type of error a few times. Let's focus on it!\n\n**Focus Area: ${errorType.replace('_', ' ')}**\n${extraInfo}`;
};

export const processMessage = async (userId: number, text: string): Promise<string> => {
    ...
    if (cachedCorrection) {
        ...
        correction = await getCorrection(text, user.language);
    }
    ...
    return reinforcementMessage ? `${reinforcementMessage}\n\n${regularResponse}` : regularResponse;
};
