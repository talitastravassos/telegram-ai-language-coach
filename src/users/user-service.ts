...
export const incrementErrorCount = async (userId: number, errorType: string): Promise<number> => {
    const client = getRedisClient();
...
export const getErrorHistory = async (userId: number): Promise<Record<string, number>> => {
    const client = getRedisClient();
    const errorsKey = `${USER_ERRORS_PREFIX}${userId}`;
...
export const resetErrorCount = async (userId: number, errorType: string): Promise<void> => {
    const client = getRedisClient();
    const errorsKey = `${USER_ERRORS_PREFIX}${userId}`;
